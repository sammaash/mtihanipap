...
if(@$_GET['demaili']) {
$demail=@$_GET['demaili'];
$r1 = mysqli_query($con,"DELETE FROM courses WHERE code='$demaili' ") or die('Error');
...
if(isset($_SESSION['key'])){
if(@$_GET['demailie']) {
$demail=@$_GET['demailie'];
$r1 = mysqli_query($con,"DELETE FROM admin WHERE colno='$demaili' ") or die('Error');
header("location:dash.php?q=31");
}
}
...
$q3=mysqli_query($con,"INSERT INTO quiz VALUES  ('$id','$name' , '$sahi' , '$wrong','$total','$time' ,'$desc','$tag', '$date','$faculty','$course')");
header("location:dash.php?q=4&step=2&eid=$id&n=$total");
/*$result = mysqli_query($con,"SELECT * FROM user WHERE course='$course' and facaulty='$faculty' ") or die('Error');
while($row = mysqli_fetch_array($result)) {
	session_start();
	$phone = $row['mob'];
	$namey = $row['name'];
	$message = "Hello $namey, head over to your portal and check your $name exam";
$_SESSION['pno']=$phone;
$_SESSION['message']=$message;
header('Location: sms2.php');
echo "<script type='text/javascript'>
alert('$message');
window.location.href='location:dash.php?q=4&step=2&eid=$id&n=$total';
</script>";
}*/
...
if($sn == 1)
{
$q=mysqli_query($con,"INSERT INTO history VALUES('$email','$eid' ,'0','0','0','0',NOW(),'$course','$faculty')")or die('Error');
}
...
if($sn == 1)
{
$q=mysqli_query($con,"INSERT INTO history VALUES('$email','$eid' ,'0','0','0','0',NOW(),'$course','$faculty')")or die('Error137');
}
...
if($rowcount == 0)
{
$q2=mysqli_query($con,"INSERT INTO rank VALUES('$email','$s',NOW(),'$faculty','$course')")or die('Error165');
}
else
{
while($row=mysqli_fetch_array($q) )
...
$sun=$sun-$s;
$q=mysqli_query($con,"UPDATE `rank` SET `score`=$sun ,time=NOW(),'faculty'=$faculty, 'course'=$course WHERE email= '$email'")or die('Error174');
